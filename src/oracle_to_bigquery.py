...
def query_factory(schema: str, exclude: str = None) -> str:
    if exclude != "":
        query = "SELECT table_name FROM user_tables where owner = '%s' and table_name not in (%s)" % (schema, exclude)
    else:
        query = "SELECT table_name FROM user_tables where owner = '%s'" % schema
    return query
